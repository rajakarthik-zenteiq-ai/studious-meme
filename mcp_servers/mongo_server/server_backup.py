"""
MongoDB FastMCP server - Production-ready with comprehensive tools and Linode Object Storage integration
...
class DatabaseManager:
    """Manages MongoDB connection lifecycle with connection pooling and retry logic, plus Linode Object Storage"""
    ...
    async def initialize(self):
        """Initialize database connection, Linode Object Storage, and indexes"""
    ...
    async def setup_storage(self):
        """Setup Linode Object Storage client"""
        try:
            self.storage = LinodeObjectStorage()
            logger.info("✅ Linode Object Storage client initialized")
        except Exception as e:
            logger.warning(f"Linode Object Storage setup failed: {e} - will use GridFS fallback")
            self.storage = None
...
async def upload_file(request: FileUploadRequest, ctx: Context) -> Dict[str, Any]:
    ...
    - Automatic fallback to GridFS if Linode Object Storage is unavailable
    ...
    try:
        ...
        if file_ext in dataset_extensions:
            ...
            bucket_name = "models"
            is_dataset = False
        elif file_ext in document_extensions or file_ext in image_extensions:
            bucket_name = "files"
            is_dataset = False
        else:
            bucket_name = "files"  # Default bucket for other file types
            is_dataset = False
        
        # Additional dataset detection for CSV/JSON files
        if file_ext in ['.csv', '.json', '.txt'] and len(file_content) > 0:
            try:
                # Quick content analysis to confirm it's a dataset
                content_preview = file_content[:1024].decode('utf-8', errors='ignore')
                if file_ext == '.csv':
                    # Check for CSV structure (commas, headers)
                    lines = content_preview.split('\n')[:5]
                    if len(lines) >= 2 and ',' in lines[0]:
                        is_dataset = True
                elif file_ext == '.json':
                    # Check for JSON array/object structure
                    import json
                    try:
        ...
        # Upload to Linode Object Storage if available
        storage_path = None
        storage_success = False
        
        if db_manager.storage:
            try:
                # Determine S3 key with directory prefix
                s3_key = f"{bucket_name}/{file_id}"
                
                # Upload to Linode Object Storage
                storage_url = db_manager.storage.upload_bytes(
                    key=s3_key,
                    data=file_content,
                    bucket_name=None  # Use default bucket (mcp)
                )
                
                if storage_url:
                    storage_path = s3_key
                    storage_success = True
                    logger.info(f"✅ File uploaded to Linode Object Storage: {storage_path}")
        ...
        # Fallback to GridFS if Linode Object Storage failed or unavailable
        if not storage_success and db_manager.gridfs:
            try:
                ...
                storage_path = f"gridfs://{gridfs_id}"
                storage_success = True
                logger.info(f"✅ File uploaded to GridFS: {storage_path}")
            except Exception as e:
                logger.error(f"GridFS upload failed: {e}")
        
        if not storage_success:
            return {"success": False, "error": "Failed to store file in both Linode Object Storage and GridFS"}
        ...
        metadata_doc = {
            ...
            "bucket": bucket_name if storage_path and not storage_path.startswith("gridfs://") else "gridfs",
            "metadata": request.metadata,
            "is_dataset": is_dataset,  # Enhanced dataset detection
            "file_extension": file_ext,
            "auto_detected_type": "dataset" if is_dataset else ("model" if "models" in bucket_name else "document")
        }
        ...
        return {
            ...
            "content_type": request.content_type,
            "storage_path": storage_path,
            "bucket": bucket_name if not storage_path.startswith("gridfs://") else "gridfs"
        }
...
async def download_file(request: Dict[str, Any]) -> Dict[str, Any]:
    """
    Download a file from Linode Object Storage or GridFS storage.
    ...
    2. Retrieves from Linode Object Storage if available
    3. Falls back to GridFS if Linode Object Storage is unavailable
    ...
    try:
        ...
        if storage_path and storage_path.startswith("gridfs://"):
        ...
        elif db_manager.storage and not storage_path.startswith("gridfs://"):
            # Linode Object Storage
            try:
                # Use the storage_path directly (already includes directory prefix)
                ...
                if file_content is None:
                    return {
                        "success": False, 
                        "error": "File not found in Linode Object Storage",
                        "details": {
                            "storage_type": "linode_s3",
                            "storage_path": storage_path,
                            "file_id": file_id,
                            "suggestion": "Check if file was uploaded successfully to S3"
                        }
                    }
            except ClientError as e:
                logger.error(f"Linode Object Storage download failed: {e}")
                return {
                    "success": False, 
                    "error": f"Linode Object Storage download failed: {str(e)}",
                    "details": {
                        "storage_type": "linode_s3",
                        "storage_path": storage_path,
                        "file_id": file_id,
                        "suggestion": "Check if Linode Object Storage service is running and accessible"
                    }
                }
            ...
            return {
                ...
                "details": {
                    "storage_available": db_manager.storage is not None,
                    "storage_path": storage_path,
                    "suggestion": "Check if Linode Object Storage or GridFS storage is properly configured"
                }
            }
        ...
        return {
            ...
            "storage_type": "linode_s3" if not storage_path.startswith("gridfs://") else "gridfs"
        }     
...
async def list_available_datasets(
    ...
    """
    List available datasets from Linode Object Storage and MongoDB collections with user isolation.
    
    This tool provides a comprehensive view of datasets accessible to the authenticated user:
    - Automatically extracts user_id from request headers for authentication
    - Scans Linode Object Storage buckets for user's uploaded files (CSV, JSON, TXT)
    ...
    - Categorizes datasets by source (Linode Object Storage, MongoDB collections)
    
    Args:
        ctx: Request context containing user authentication headers
        bucket_name: Linode Object Storage bucket to scan (default: "datasets")
    ...
    try:
        ...
        # List from Linode Object Storage if available
        if db_manager.storage:
            try:
                objects = db_manager.storage.list_objects(bucket_name)
                storage_datasets = []
                for obj in objects:
                    # Only show files that belong to the authenticated user
                    if obj.get("key", "").startswith(f"{final_user_id}_"):
                        dataset_info = {
                            "id": obj["key"],
                            "name": obj["key"].split("_", 2)[-1] if "_" in obj["key"] else obj["key"],
                            "size": obj.get("size", 0),
                ...
                logger.info(f"Found {len(storage_datasets)} datasets in Linode Object Storage")
                
            except ClientError as e:
                logger.warning(f"Linode Object Storage list error (bucket may not exist): {e}")
            except Exception as e:
                logger.warning(f"Linode Object Storage access error: {e}")
        ...
        return {
            ...
            "sources": {
                "linode_s3": len([d for d in available_datasets if d.get("source") == "linode_s3"]),
...
async def get_dataset_info(dataset_id: str, ctx: Context) -> Dict[str, Any]:
    ...
    - Searches both Linode Object Storage and MongoDB metadata
    ...
    try:
        ...
        # Check Linode Object Storage first (validate ownership via naming convention)
        if db_manager.storage:
            try:
                if not dataset_id.startswith(f"{user_id}_"):
                    return {"success": False, "error": "Access denied - dataset belongs to different user", "dataset_id": dataset_id, "user_id": user_id}
                # Try datasets bucket
                obj_metadata = db_manager.storage.get_object_metadata(dataset_id, "datasets")
                if obj_metadata:
                    dataset_info = {
                        "id": dataset_id,
                        "name": dataset_id,
                    ...
                    # Try files bucket
                    obj_metadata = db_manager.storage.get_object_metadata(dataset_id, "files")
                    if obj_metadata:
                        dataset_info = {
                            "id": dataset_id,
                            "name": dataset_id,
        ...
        # Check MongoDB if not found in object storage
        if dataset_info is None:
            try:
                metadata = await db_manager.db["file_metadata"].find_one({"file_id": dataset_id, "user_id": user_id})
                if metadata:
                    dataset_info = {
                        "id": dataset_id,
                    ...
                    if dataset_id in collections:
                        ...
                        if user_count > 0:
                            ...
                            return {"success": False, "error": f"Access denied - no data found for user in collection '{dataset_id}'", "user_id": user_id}
            except Exception as e:
                logger.warning(f"MongoDB lookup error: {e}")
        
        if dataset_info is None:
            return {"success": False, "error": f"Dataset '{dataset_id}' not found or access denied", "details": {"dataset_id": dataset_id, "user_id": user_id, "searched_in": ["Linode datasets bucket", "Linode files bucket", "MongoDB file_metadata", "MongoDB collections"], "suggestion": "Check if dataset_id is correct and belongs to your user account"}}
...
async def sync_file_metadata() -> Dict[str, Any]:
    """Synchronize file metadata with storage backends (Linode S3 + GridFS)."""
    await ensure_db_initialized()
    if db_manager.db is None:
        return {"success": False, "error": "Database not initialized"}
    if not await db_manager.check_connection():
        return {"success": False, "error": "Database not connected"}
    try:
        sync_results = {"total_metadata_entries": 0, "verified_files": 0, "orphaned_metadata": 0, "cleaned_up": 0, "errors": []}
        ...
        for metadata_doc in metadata_docs:
            ...
            try:
                if storage_path and storage_path.startswith("gridfs://"):
                    ...
                    obj_id = ObjectId(gridfs_id)
                    grid_out = await db_manager.gridfs.open_download_stream(obj_id)
                    await grid_out.read(1)
                    file_exists = True
                elif db_manager.storage:
                    exists = db_manager.storage.object_exists(file_id, bucket)
                    if exists:
                        file_exists = True
                if file_exists:
                ...
                sync_results["orphaned_metadata"] += 1
        if orphaned_ids:
            result = await db_manager.db["file_metadata"].delete_many({"_id": {"$in": orphaned_ids}})
...
async def repair_file_storage() -> Dict[str, Any]:
    ...
    if db_manager.db is None:
        return {"success": False, "error": "Database not initialized"}
    try:
        repair_results = {"issues_found": [], "repairs_attempted": [], "success_count": 0, "error_count": 0}
        ...
        if db_manager.storage:
            try:
                # Attempt to list objects in datasets bucket (non-fatal if fails)
                db_manager.storage.list_objects("datasets")
                repair_results["repairs_attempted"].append("Linode Object Storage connectivity verified")
                repair_results["success_count"] += 1
            except Exception as e:
                storage_issues.append(f"Linode Object Storage connectivity issue: {str(e)}")
                repair_results["issues_found"].append(f"Linode Object Storage problem: {str(e)}")
                repair_results["error_count"] += 1
        else:
            storage_issues.append("Linode Object Storage client not initialized")
            repair_results["issues_found"].append("Linode Object Storage client not available")
            repair_results["error_count"] += 1
        try:
        ...
        repair_results["summary"] = f"{repair_results['success_count']} checks passed, {repair_results['error_count']} issues found"
        if storage_issues:
            repair_results["storage_issues"] = storage_issues
        return repair_results
    except Exception as e:
        logger.error(f"Error repairing file storage: {e}")
        return {"success": False, "error": f"Repair error: {str(e)}"}

@mcp.tool()
async def data_inspect(request: Dict[str, Any]) -> Dict[str, Any]:
    """Inspect a stored file (dataset or document) and return a high-level summary.
    Input:
      file_id: ID returned from upload_file or listing tools (required)
      sample_rows: int (optional, default 5) – number of rows to sample for tabular data
    Behavior:
      - For CSV/XLSX: load via pandas, compute shape, dtypes, null counts, basic stats (numeric), sample rows
      - For JSON (array/dict): show top-level keys and length
      - For TXT/LOG: return first N lines
      - For PDF / images: if docling installed, attempt structured extraction summary
    """
    await ensure_db_initialized()
    file_id = request.get("file_id")
    sample_rows = int(request.get("sample_rows", 5))
    if not file_id:
        return {"success": False, "error": "file_id is required"}
    try:
        ...
        content_resp = await download_file({"file_id": file_id, "user_id": meta.get("user_id")})
        if not content_resp.get("success"):
            return {"success": False, "error": f"Unable to retrieve file: {content_resp.get('error')}"}
        content_type = meta.get("content_type", "")
        filename = meta.get("filename", "")
        ext = os.path.splitext(filename)[1].lower()
        import base64, io, json
        raw_bytes = base64.b64decode(content_resp["content"]) if "content" in content_resp else b""
        ...
        if ext in [".csv", ".tsv", ".xlsx", ".xls"]:
            ...
            try:
                if ext in [".csv", ".tsv"]:
                    sep = "\t" if ext == ".tsv" else ","
                    df = pd.read_csv(io.BytesIO(raw_bytes), sep=sep, nrows=10000)
                else:
                    df = pd.read_excel(io.BytesIO(raw_bytes), nrows=10000)
                summary.update({
                    "type": "tabular",
                    "rows": int(df.shape[0]),
                    "columns": int(df.shape[1]),
                    "column_names": list(df.columns)[:200],
                    "dtypes": {c: str(t) for c, t in df.dtypes.items()},
                    "null_counts": {c: int(df[c].isna().sum()) for c in df.columns[:200]},
                })
                # Basic stats for numeric columns
                numeric_df = df.select_dtypes(include=["number"])  # type: ignore
                if not numeric_df.empty:
                    desc = numeric_df.describe().to_dict()
                    summary["numeric_stats"] = {k: {m: float(v) for m, v in stats.items()} for k, stats in desc.items()}
                summary["sample"] = df.head(sample_rows).to_dict(orient="records")
            except Exception as e:
                summary["parse_error"] = f"Tabular parse failed: {e}";
        # JSON structures
        elif ext in [".json"]:
            try:
                ...
                if isinstance(data, list):
                    ...
                    summary.update({
                        "type": "json_object",
                        "top_level_keys": list(data.keys())[:200]
                    })
                else:
                    summary.update({"type": "json_primitive", "repr": str(data)[:500]})
            except Exception as e:
                summary["parse_error"] = f"JSON parse failed: {e}";
        # Plain text / log
        elif ext in [".txt", ".log"]:
            text = raw_bytes.decode("utf-8", errors="replace")
        ...
        # PDF / Images via docling (if available)
        elif ext in [".pdf", ".png", ".jpg", ".jpeg"]:
            try:
                from docling.document import Document
                from docling.parsers import DocumentParser
                dp = DocumentParser()
                doc: Document = dp.parse(io.BytesIO(raw_bytes))
                summary.update({
                    "type": "document",
                    "pages": len(doc.pages),
                    "elements": len(doc.elements),
                    "sample_text": doc.export_text()[:1000]
                })
            except Exception as e:
                summary.update({"type": "binary", "note": f"Docling parse unavailable or failed: {e}"})
        else:
            summary.update({"type": "binary", "note": "Unsupported inspection type"})
        return {"success": True, "summary": summary}
    except Exception as e:
        logger.error(f"data_inspect error: {e}")
